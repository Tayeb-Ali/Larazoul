...
namespace Larazoul\Larazoul\Chumper\Zipper;

use Larazoul\Larazoul\Chumper\Zipper\Repositories\RepositoryInterface;
use Exception;
...
class Zipper
{
    ...
    public function make($pathToFile, $type = 'zip')
    {
        ...
        $objectOrName = $type;
        if (is_string($type)) {
            $objectOrName = 'Larazoul\Larazoul\Chumper\Zipper\Repositories\\' . ucwords($type) . 'Repository';
        }

        if (!is_subclass_of($objectOrName, 'Larazoul\Larazoul\Chumper\Zipper\Repositories\RepositoryInterface')) {
