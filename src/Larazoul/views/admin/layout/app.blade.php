@php
    $adminMenu = \Larazoul\Larazoul\Larazoul\Models\MenuItem::where('menu_id' , 1)
        ->orderBy('order')
        ->where('parent_id' ,0)
        ->with('parent')
        ->get();
...
<head>
    ...
    <title>@lang('larazoul::larazoul.Larazoul')</title>
                    ...
                    <li class="{{ $hasChild === true ? "treeview" : "" }} {{ $segment == strtolower($menu->slug) ? "menu-open" : "" }}">
                        ...
                        @if($hasChild )
                            <ul class="treeview-menu"
                                style="{{ $segment == strtolower($menu->slug) ? "display: block;" : "" }}">
