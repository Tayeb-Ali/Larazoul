...
@php $selectedArray = isset($selectedArray) ? $selectedArray : (old($name) ? (array) old($name) :  []) @endphp
...
<div class="form-group {{ $errors->has($name) ? "has-error" : ""  }} {{ isset($divClass) ? $divClass : '' }}" id="{{ isset($divId) ? $divId : '' }}" data-key="{{ isset($key) ? $key : '' }}">
    ...
    @if(isset($array))
        @foreach($array as $key => $value)
               {!!  Form::radio($name , $key, in_array($key , $selectedArray) , ['id' => $value , 'class' => $inputClass]) !!}
